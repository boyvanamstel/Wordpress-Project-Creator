...
require_once('WPProjectCreator.php');
...
<?php	
	...
	$settings['wp_url'] = isset($_POST['wp_url']) ? $_POST['wp_url'] : str_replace('/wp-content/wpprojectcreator','',WPProjectCreator::getCurrentPageURL(true));
	$settings['db_name'] = isset($_POST['db_name']) ? $_POST['db_name'] : '';
	$settings['db_dump'] = isset($_POST['db_dump']) ? $_POST['db_dump'] : '';
	...
	if(!isset($_POST['setup_submit'])) {
		//$existingSettings = file_exists('../wp-config.php') ? //WPProjectCreator::readWPConfig('../wp-config', true) : array();
				...
				if($wpProjectCreator->createDBConnection()) {
					...
					if($wpProjectCreator->createEmptyDatabase()) {
						...
						if($wpProjectCreator->importDatabase()) {
							?><li>Database imported, proceed to <a href="<?php echo $settings['wp_url']; ?>">website</a></li><?php
